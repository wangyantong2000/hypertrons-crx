...
export const useInteractStatus = (interactModel: InteractModel, option: ControlledValueOption = {}) => {
  const statusReducer = (state: InteractStatus, payload: StatusDispatch): InteractStatus => {
    ...
    switch (payload.type) {
      ...
      // 处理 元素本身的点击状态
      case 'handleClick':
        const currentNode = interactModel.getRuleAndIndexByElement(payload.target);
      ...
      // 处理 hover 状态
      case 'handleHover':
        const node = payload.target && interactModel.getRuleAndIndexByElement(payload.target);
        return {
          ...status,
