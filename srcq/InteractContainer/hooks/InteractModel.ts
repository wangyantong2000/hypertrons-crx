...
export class InteractModel {
  ...
  getElementByStatus(status: InteractStatus): HTMLElement {
    ...
    const model = this.models.find((m) => m.id === rule.id && m.actions.includes(status.action));
   ...
   * 获取规则匹配的所有元素
   */
  static getValidElements(rule: CanvasInteractRule, container: ContainerNode = document) {
   ...
   * @private
   */
  static querySelector(selector: string, container: Document | Element = document) {
    // 先用约定标识符
  ...
  initListener = (ele: Element[] | ContainerNode, type: keyof GlobalEventHandlersEventMap, method) => {
   ...
   * @param hover
   */
  getInteractListeners({ click, hover }: { click; hover: { onHoverStart; onHoverEnd } }) {
    const clickFn = this.getClickListeners(click);
    const hoverFn = this.getHoverListeners(hover.onHoverStart, hover.onHoverEnd);
