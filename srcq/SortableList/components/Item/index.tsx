            ...
            {renderItem ? (
              ...
              <Flexbox className={styles.content} direction={'horizontal'} align={'center'}>
                ...
                <Flexbox flex={1} style={{ paddingLeft: 4 }}>
                  {renderContent ? renderContent(item, index) : <div>{typeof item === 'string' ? item : index}</div>}
                </Flexbox>
                <Flexbox className={classNames(styles.actions)} direction={'horizontal'} align="center">
                  {typeof actions === 'function' ? actions(item, index) : actions}
                  {hideRemove ? null : <DeleteAction tabIndex={-1} onClick={onRemove} style={{ height: 22 }} />}
                </Flexbox>
              </Flexbox>
            )}
          </div>
