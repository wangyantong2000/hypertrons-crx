...
const TreeNode = memo<TreeNodeProps>(({ node, virtualStyle }) => {
  ...
  return (
    <SortableTreeItem
      ...
      // 交互操作
      onCollapse={children.length ? () => dispatchTreeData({ type: 'toggleCollapse', id }) : undefined}
      node={node}
...
const TreeList = memo(() => {
  const [dispatchTreeData, hideAdd, virtual] = useStore((s) => [s.dispatchTreeData, s.hideAdd, s.virtual], shallow);
