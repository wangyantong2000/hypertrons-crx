...
const useStyles = createStyles(({ css, cx, prefixCls, token }, { collapsed }: { collapsed: boolean }) => {
...
const TreeItem: FC<TreeItemProps> = memo(
  ({
    ...
    const [indentationWidth, selected, Content, Extra, withKeyboardSelectNode, deselectedNode, disableDrag] = useStore(
      (s) => [
        s.indentationWidth,
      ...
      <>
        ...
        >
          <div className={`${nodePrefix}-body`} ref={setDraggableNodeRef} style={style}>
            ...
            {clone && childCount && childCount > 1 ? <span className={`${nodePrefix}-count`}>{childCount}</span> : null}
          </div>
          {extraPanelVisible ? <div className={styles.extra}>{Extra && <Extra {...node} />}</div> : null}
        </li>
      </>
    );
  }
