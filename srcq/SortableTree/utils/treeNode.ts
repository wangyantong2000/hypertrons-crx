...
const flatten = (items: TreeData, parentId: UniqueIdentifier | null = null, depth = 0): FlattenNode[] =>
  items.reduce<FlattenNode[]>((acc, item, index) => {
    return [...acc, { ...item, parentId: parentId, depth, index }, ...flatten(item.children, item.id, depth + 1)];
  }, []);
