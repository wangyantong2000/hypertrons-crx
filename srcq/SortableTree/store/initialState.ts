import type { FlattenNode, Projected, RenderNodeProps, TreeData, UniqueIdentifier, VirtualConfig } from '../types';
import { TreeDataDispatchPayload } from './treeDataReducer';

export type OnTreeDataChange<T = any> = (treeData: TreeData<T>, event: TreeDataDispatchPayload) => void;
...
export interface State<T = any> extends ControlledState {
   ...
   */
  sortableRule?: (data: { activeNode: FlattenNode<T>; targetNode: FlattenNode<T>; projected: Projected }) => boolean;
  /**
   * 虚拟滚动配置
   */
...
export const initialDragState: Pick<State, 'offsetLeft' | 'overId' | 'activeId' | 'currentPosition'> = {
  offsetLeft: 0,
  overId: null,
  activeId: null,
