...
export type CreateAssetStore<T> = StateCreator<T & ProBuilderInstance, [['zustand/devtools', never]], [], T>;

const { Provider, useStore, useStoreApi } = createContext<StoreApi<any>>();

export const createAssetStore = <T>(createStore: StateCreator<T>, options?: AssetStoreOptions<T>) => {
  const store = () => {
...
export const createUseAssetStore = <T>(): {
  ...
  AssetProvider: ({ createStore, children }: { createStore: () => StoreApi<T>; children: any }) => ReactNode;
