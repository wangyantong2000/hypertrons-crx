...
export class ComponentAsset<Config = any, Props = any> {
  ...
  constructor(params: ComponentAssetParams<Config>) {
    ...
    this.configSelector = params.storeOptions?.getConfig || ((s: Config) => JSON.parse(JSON.stringify(s)));
   ...
   * @param type
   */
  generateProps = (config: Partial<Config>, type: EmitterEnv = EmitterEnv.Runtime): Partial<Props> => {
    const props = {};
