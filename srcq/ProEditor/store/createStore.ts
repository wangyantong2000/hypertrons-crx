...
const vanillaStore: StateCreator<InternalProEditorStore, [['zustand/devtools', never]]> = (...params) => ({
  ...generalSlice(...params),
  ...configSlice(...params),
});
...
export const createStore = (options: boolean | DevtoolsOptions = false) => {
  ...
  const devtoolOptions = options === false ? undefined : options === true ? { name: 'ProEditorStore' } : options;

  return createWithEqualityFn<InternalProEditorStore>()(devtools(vanillaStore, devtoolOptions), isEqual);
};
