...
describe('ProEditorProvider', () => {
  ...
  it('should render Store correctly', () => {
    const Provider = ({ children }) => <ProEditorProvider store={[useAStore, useBStore]}>{children}</ProEditorProvider>;
  ...
  it('嵌套时使用同一个 Store', () => {
    const Provider = ({ children }) => <ProEditorProvider store={[useAStore]}>{children}</ProEditorProvider>;
