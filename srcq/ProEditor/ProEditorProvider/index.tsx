...
export const ProEditorProvider: FC<ProEditorProviderProps> = ({ children, devtoolOptions, store }) => {
  let isWrapped = true;

  const Content = (
    <>
      {children}
      {store?.map((item, index) => <StoreUpdater key={`${item.name}-${index}`} store={item} />)}
    </>
  );
