 ...
 */
export interface ProEditorInstance<Config = any, Props = any> extends ConfigPublicAction, GeneralPublicAction {
  /**
   * 获取配置信息
   * @returns {Config} - 配置信息
   */
...
export const useProEditor = <T>(): ProEditorInstance<T> => {
  ...
  const { undoStack, undoLength, redoLength, redoStack, setConfig, exportConfig, resetConfig, undo, redo } =
    storeApi.getState();
