  ...
  ({ item, index, columns, listeners, actions, hideRemove, dragging, creatorButtonProps }) => {
    ...
    return (
      <Flexbox className={styles.item} direction={'horizontal'} align={'center'}>
        ...
        <Flexbox horizontal gap={4} width={'100%'} className={styles.content}>
          {columns.map((col, colIndex) => {
            ...
            switch (col.type) {
              ...
              case 'select':
                return <ControlSelect key={`${item?.dataIndex}-${colIndex}`} options={col.options} {...props} />;
              case 'custom':
        ...
        <Flexbox className={classNames(styles.actions, styles.actionsRight)} direction={'horizontal'} align="center">
          {actions}
          {hideRemove ? null : (
            <DeleteAction
              tabIndex={-1}
