...
export const configSlice: StateCreator<InternalProBuilderStore, [['zustand/devtools', never]], [], ConfigSlice> = (
  set,
  get
) => {
  ...
  return {
    ...
    setConfig: (config, { replace, recordHistory } = {}) => {
      get().internalUpdateConfig(config, { type: '调用 updateConfig 更新', payload: config }, replace);
