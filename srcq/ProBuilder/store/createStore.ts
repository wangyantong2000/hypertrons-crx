...
const vanillaStore: StateCreator<InternalProBuilderStore, [['zustand/devtools', never]]> = (...params) => ({
  ...generalSlice(...params),
  ...configSlice(...params),
  ...canvasSlice(...params),
...
export const createStore = (options: boolean | DevtoolsOptions = false) => {
  ...
  const devtoolOptions = options === false ? undefined : options === true ? { name: 'ProBuilderStore' } : options;

  return createWithEqualityFn<InternalProBuilderStore>()(devtools(vanillaStore, devtoolOptions), isEqual);
};
