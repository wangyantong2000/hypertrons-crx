import { displayListSelector, iconfontScriptsChangeable, isEmptyIconfontScripts, isEmptyIconList } from './selectors';
...
test('isEmptyIconfontScripts', () => {
  ...
  // 3 有值
  expect(isEmptyIconfontScripts({ iconfontScripts: [{}] } as Store)).toBeFalsy();
});
...
test('isEmptyIconList', () => {
  ...
  // 2 为空
  expect(isEmptyIconList({ iconfontIconList: [], antdIconList: [] } as Store)).toBeTruthy();
