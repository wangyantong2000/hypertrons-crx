...
export const isEmptyIconfontScripts = (s: Store) => !s.iconfontScripts || s.iconfontScripts.length === 0;
...
export const displayListSelector = (s: Store) => {
  ...
  return list.filter((i) => {
    ...
    switch (i.type) {
      ...
      case 'iconfont':
        return i.props.type.toLowerCase().includes(filterKeywords.toLowerCase());
    }
  });
};
